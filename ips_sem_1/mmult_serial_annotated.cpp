...
void multiply_d(int arrSize, double **aMatrix, double **bMatrix, double **product)
{
    for(int i=0;i<arrSize;i++) {
        for(int j=0;j<arrSize;j++) {
            double sum = 0;
            ANNOTATE_SITE_BEGIN(loop)
            for(int k=0;k<arrSize;k++) {
                ANNOTATE_TASK_BEGIN(sum)
                sum += aMatrix[i][k] * bMatrix[k][j];
                ANNOTATE_TASK_END(sum)
            }
            ANNOTATE_SITE_END(loop)
			product[i][j] = sum;
        }
    }
}
...
int main(int argc, char*argv[])
{
    ...
    if(argc !=2) {
        cerr << "Usage: mmult_serial_annotate.exe arraySize [default is 1024].\n";
        num = 1024;
    } else {
        num = atoi(argv[1]);
        if (num < 0) {
    ...
    // print elapsed time
    cout << "Elapsed time = " << ((double)(stop - start)) / CLOCKS_PER_SEC << " seconds" << endl;
